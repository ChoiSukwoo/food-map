...
import { generateNewFeedback, MAX_CONTENT_LENGTH, SNAIL_SIDE_LENGTH } from '@utils/feedback';
...
const FeedbackSection = ({ initialFeedbackList }: Props): React.ReactElement => {
  ...
  // 새로운 피드백
  const [newContent, onChangeContent, setContent] = useInput('', MAX_CONTENT_LENGTH);

  return (
    <>
      <FeedbackBoardContainer showClones={showClones}>
        <>
          <FourFakeFeedbackBoards feedbackList={feedbackList} showClones={showClones} />
          <FeedbackBoard feedbackList={feedbackList} feedbackContent={newContent} onChangeContent={onChangeContent} />
          <FourFakeFeedbackBoards feedbackList={feedbackList} showClones={showClones} />
        </>
      </FeedbackBoardContainer>
      <FeedbackSubmitButton newContent={newContent} setFeedbackList={setFeedbackList} setContent={setContent} />
    </>
  );
};
...
const FourFakeFeedbackBoards = ({ feedbackList, showClones }: FakeFeedbackBoardProps) =>
  showClones ? (
    <>
